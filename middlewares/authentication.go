...
func CheckAuthentication() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if c.Request.URL.Path == "/login" || c.Request.URL.Path == "/logout" || c.Request.URL.Path == "/register" || c.Request.URL.Path == "/auth/verification" || c.Request.URL.Path == "/health" || c.Request.URL.Path == "/support/contact" {
			c.Next()
			return
		}
